...
int main(int argc, char* argv[]) {
    ...
    // Register built-in task types manually (heartbeat and sleep)
    [[maybe_unused]] bool registered_heartbeat = taskRegistry->register_task_type("heartbeat", [](const std::string& /*id*/, const nlohmann::json& params) {
        int interval_sec = 5;
        if (params.contains("interval")) {
            interval_sec = params["interval"].get<int>();
        }
        return std::make_unique<HeartbeatTask>(std::chrono::seconds(interval_sec));
    });
    
    [[maybe_unused]] bool registered_sleep = taskRegistry->register_task_type("sleep", [](const std::string& /*id*/, const nlohmann::json& params) {
        int duration_sec = 10;
